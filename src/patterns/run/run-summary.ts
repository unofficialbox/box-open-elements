...
export class RunSummary extends BaseElement {
  ...
  protected renderTemplate(): void {
    ...
    button{font:inherit;color:inherit;background:none;border:0;cursor:pointer;padding:.5rem 0;display:flex;gap:.5rem;align-items:center}
    ...
    <button part="trigger" aria-expanded="false" aria-controls="details"><box-status-icon></box-status-icon><span part="label" role="status"></span><span part="clock" aria-hidden="true"></span><span aria-hidden="true">›</span></button>
    <div id="details" part="disclosure" class="boe-disclosure" inert><div><ul part="plan"></ul><box-run-trace></box-run-trace></div></div>`;
  }
  ...
  private updateLabel(): void {
    ...
    (this.shadowRoot!.querySelector("box-status-icon") as StatusIcon).kind = p.kind;
