...
import { hasEndArrow, hasStartArrow, resolveGraphArrows } from "./graph-arrows.js";
import type { GraphArrows } from "./graph-arrows.js";
...
export class VersionGraph extends BaseElement {
  ...
  static get observedAttributes(): string[] {
    return ["arrows", "heading", "versions"];
  }

  /**
   * Which ends of each edge carry an arrowhead. Defaults to `end`, the head on
   * the child, because the layout builds edges parent → child.
   */
  ...
  protected update(): void {
    ...
    const startMarker = hasStartArrow(arrows) ? ` marker-start="url(#boe-graph-arrow)"` : "";
    const endMarker = hasEndArrow(arrows) ? ` marker-end="url(#boe-graph-arrow)"` : "";
    ...
    const edgePaths = layout.edges
      .map(edge => {
        ...
        return `<path part="edge" data-from="${escapeHtml(edge.fromId)}" data-to="${escapeHtml(edge.toId)}" d="${d}"${startMarker}${endMarker}></path>`;
      })
      .join("");
