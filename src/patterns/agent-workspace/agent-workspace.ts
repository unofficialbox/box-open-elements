...
import { boeReducedMotionPolicy } from "../../foundations/motion/index.js";
...
export class AgentWorkspace extends BaseElement {
  ...
  static get observedAttributes(): string[] { return ["viewer-key"]; }
  ...
  private toggle(pane: "chats" | "details"): void {
    if (pane === "chats") {this.chatsOpen = !this.chatsOpen; if (this.mobile) this.detailsOpen = false;}
    else {this.detailsOpen = !this.detailsOpen; if (this.mobile) this.chatsOpen = false;}
    ...
    this.syncPanes();
    if (this.mobile && (this.chatsOpen || this.detailsOpen)) this.shadowRoot!.querySelector<HTMLElement>(`#${pane} button`)?.focus({preventScroll:true});
  }
  ...
  protected renderTemplate(): void {
    ...
    *{box-sizing:border-box} section{position:relative;overflow:clip;height:100%;display:flex;min-width:0}
    aside{width:0;overflow:clip;transition:width var(--boe-profile-motion-panel,320ms) ease-out;background:var(--boe-token-surface-surface,#fff)} aside[data-open]{width:280px;flex-shrink:0}
    ...
    #scrim{position:absolute;inset:0;background:#0005;border:0;border-radius:0;z-index:2} [hidden]{display:none!important}
    ...
    <main><nav><button data-toggle="chats" aria-controls="chats">Conversations</button><button data-toggle="details" aria-controls="details">Details</button></nav><div id="conversation" part="conversation"><slot name="conversation"><div id="sessions"></div></slot></div></main>
  ...
  protected update(): void {
    ...
    for (const summary of this.model?.summaries ?? []) {
      ...
      b.addEventListener("click", () => {this.model?.select(summary.id); if (this.mobile) this.closeDrawers();}); list.append(b);
      if (summary.id === activeRow) b.focus({preventScroll:true});
    }
    for (const [id, element] of this.sessions) if (!this.model?.chats.some(c => c.id === id && c.controller === element.chatController)) {element.remove(); this.sessions.delete(id);}
    for (const chat of this.model?.chats ?? []) {
