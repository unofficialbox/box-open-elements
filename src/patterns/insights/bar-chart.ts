...
export class BoxBarChartElement extends HTMLElement {
  ...
  get actions(): BarChartAction[] {
    return this.parseJsonAttribute<BarChartAction[]>("actions", []);
  }

  set actions(value: BarChartAction[]) {
    this.setAttribute("actions", JSON.stringify(value));
  }

  get legend(): BarChartLegendItem[] {
    return this.parseJsonAttribute<BarChartLegendItem[]>("legend", []);
  }

  set legend(value: BarChartLegendItem[]) {
  ...
  get points(): BarChartPoint[] {
    return this.parseJsonAttribute<BarChartPoint[]>("points", []);
  }
  ...
  private emitPointSelected(point: BarChartPoint): void {
    this.dispatchEvent(
  ...
  private render(): void {
    ...
    const chartMarkup = this.points.length
      ? `
        ...
        `
      : `<div part="empty">No bar-chart data available.</div>`;
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        [part="grid-line"] {
          border-top: 1px dashed color-mix(in srgb, var(--boe-token-stroke-stroke, #d6e0ea) 62%, transparent);
        }
        ...
        [part="point"][data-tone="accent"] {
          border-color: color-mix(in srgb, #5a7cf7 22%, var(--boe-token-stroke-stroke, #d6e0ea) 78%);
          background: color-mix(in srgb, #5a7cf7 8%, white 92%);
        }

        [part="point"][data-tone="success"] {
          border-color: color-mix(in srgb, #26c281 22%, var(--boe-token-stroke-stroke, #d6e0ea) 78%);
        ...
        [part="point"][data-tone="accent"] [part="bar"] {
          ...
          box-shadow: 0 10px 18px rgba(63, 98, 234, 0.16);
        }
        ...
        [part="legend-item"][data-tone="accent"] {
          border-color: color-mix(in srgb, #5a7cf7 18%, var(--boe-token-stroke-stroke, #d6e0ea) 82%);
          background: color-mix(in srgb, #5a7cf7 7%, white 93%);
        }

        [part="legend-item"][data-tone="success"] {
          border-color: color-mix(in srgb, #26c281 18%, var(--boe-token-stroke-stroke, #d6e0ea) 82%);
