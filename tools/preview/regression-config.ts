...
/**
 * Parses the `REGRESSION_MAX_DIFF_RATIO` override into a diff ratio.
 *
 * Throws on any invalid value (empty, non-numeric, NaN, Infinity, negative)
 * rather than falling through to `NaN`: a `NaN` threshold makes every
 * `ratio > MAX_DIFF_RATIO` comparison false, which would silently disable the
 * pixel gate the CI is built around.
 */
export function parseMaxDiffRatio(raw: string | undefined): number {
  ...
  if (trimmed === "" || !Number.isFinite(parsed) || parsed < 0) {
    throw new Error(`Invalid REGRESSION_MAX_DIFF_RATIO: ${JSON.stringify(raw)}`);
  }
  return parsed;
}
