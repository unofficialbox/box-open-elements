...
- [x] Engine fixes: Sass partial resolver, `.be` boundary, nest-flatten siblings, declaration key length sort, safe `//` strip, nested at-rule parent scope
- [x] Regeneratable CSS + report via `bun run style-bridge:bue-explorer` (`tools/style-bridge/out/` is gitignored)
- [x] Tests: flatten sibling regression, partial resolver, full BUE library bridge, focused engine cases
...
- Naive `//` regex corrupts `url(//…)`; stripper must track strings and parentheses.
...
- Bridged literals use component hooks (`--boe-content-explorer-*`) or canonical `--boe-token-*` with concrete terminal fallbacks (e.g. `#e8e8e8`, `16px`).
