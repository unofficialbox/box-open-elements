...
- component gaps: `nav-sidebar`/`sidebar-toggle-button` — **done** (`box-nav-sidebar`, `box-sidebar-toggle-button`, layout tier); then `grid-view`, `fieldset`/`section`, `error-mask`, `draggable-list`, picker list items, `nudge`, `pill-cloud`, `pill-selector-dropdown`
