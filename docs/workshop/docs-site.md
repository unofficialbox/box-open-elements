...
- `bun run site:build` (→ `docs-site/build.ts`) emits `docs-site/dist/`: the bundled app (`main.js`), `styles.css`, `index.html`, the built library tree under `lib/` (the importmap target — the package boundary is preserved; the deployed app still consumes `box-open-elements` externally), and the Storybook workshop under `workshop/` (linked from the rail footer). `bun run docs:build` is the docs-only variant (workshop omitted).
