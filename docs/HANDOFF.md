...
- **BUE conformance Layer 2 — colour / shadow / interaction state** (this branch, path C): `bun run bue-conformance:color` reads the **compiled** (post-Sass, resolved) CSS of the public box-ui-elements Storybook and diffs box-open-elements' shipped colour/shadow/state values (imported from `src/foundations/tokens` + `src/foundations/geometry`, grounded by source anchors) against what upstream renders — the colour information Layer 1 defers. Report `docs/audits/bue-conformance-color-audit.md`; **button + menu-item + badge, 18 claims, 14 conformant / 4 review** (the reviews are box-open-elements' intentional Blueprint-modernised hover/text/badge-surface tones, flagged for judgement, not auto-drift). Round 2 broadened past the always-loaded bundle by reading the webpack **chunk map** from the runtime and fetching all per-story chunks, so code-split component CSS is reachable without a browser. The remaining live-browser paths in **[`plans/bue-conformance-layer2-handoff.md`](../plans/bue-conformance-layer2-handoff.md)** are now only needed for surfaces whose box-open-elements side is `color-mix()`/gradient (tooltip, some inputs/checkbox states).
