...
- **BUE conformance Layer 2 — colour / shadow / interaction state** (this branch, path C): `bun run bue-conformance:color` reads the **compiled** (post-Sass, resolved) CSS of the public box-ui-elements Storybook and diffs box-open-elements' shipped colour/shadow/state values (imported from `src/foundations/tokens` + `src/foundations/geometry`, grounded by source anchors) against what upstream renders — the colour information Layer 1 defers. Report `docs/audits/bue-conformance-color-audit.md`; **button family, 11 claims, 8 conformant / 3 review** (the 3 are box-open-elements' intentional Blueprint-modernised hover/text tones, flagged for judgement, not auto-drift). This is the browser-free path from **[`plans/bue-conformance-layer2-handoff.md`](../plans/bue-conformance-layer2-handoff.md)**; the live-browser paths (tenant login; beating the Storybook MSW service worker) remain open there for broader surface coverage.
