...
- **BUE conformance program** (merged #76): repeatable source-level geometry audit — `bun run bue-conformance` diffs `src/foundations/geometry` against real upstream box-ui-elements SCSS (pinned `v26.0.0`). Report `docs/audits/bue-conformance-audit.md`; plan `plans/bue-conformance-execplan.md`. **17/17 conformant.**
- **BUE conformance Layer 2 — colour / shadow / interaction state** (this branch, path C): `bun run bue-conformance:color` reads the **compiled** (post-Sass, resolved) CSS of the public box-ui-elements Storybook and diffs box-open-elements' shipped colour/shadow/state values (imported from `src/foundations/tokens` + `src/foundations/geometry`, grounded by source anchors) against what upstream renders — the colour information Layer 1 defers. Report `docs/audits/bue-conformance-color-audit.md`; **button + menu-item + badge + checkbox/radio + tooltip, 26 claims, 20 conformant / 6 review** (the reviews are resolved differences flagged for human judgement, never auto-drift — some read as box-open-elements' modernised button/badge tones, while others, e.g. the menu-selected tint and tooltip text, are still pending review). Round 2 broadened past the always-loaded bundle by reading the webpack **chunk map** from the runtime and fetching all per-story chunks; round 3 added a static **`color-mix(in srgb, …)` evaluator**; round 4 broadened to checkbox/radio/menu-selected/tooltip (a verbatim compound-selector matcher reaches the custom checkbox/radio marks). The remaining live-browser paths in **[`plans/bue-conformance-layer2-handoff.md`](../plans/bue-conformance-layer2-handoff.md)** are now only needed for multi-stop gradients (tooltip) and box-open-elements colours with no flat upstream counterpart.
